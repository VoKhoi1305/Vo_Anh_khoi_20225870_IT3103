...
public class CartScreenController {
	...
	// play button
	{
		...
		if (media instanceof DigitalVideoDisc) {
			...
			if (dvd.getLength() > 0)
				JOptionPane.showMessageDialog(frame, "DVD length: " + dvd.getLength(), dvd.getTitle() + " is playing",
						JOptionPane.INFORMATION_MESSAGE);
			else
				JOptionPane.showMessageDialog(frame, "ERROR: DVD length is non-positive!", dvd.getTitle() + " ERROR",
						JOptionPane.ERROR_MESSAGE);
		} else {
			CompactDisc cd = (CompactDisc) media;
			JFrame frame = new JFrame("JOptionPane showMessageDialog example");
			if (cd.getLength() > 0)
				JOptionPane.showMessageDialog(frame, "CD length: " + cd.getLength(), cd.getTitle() + " is playing",
						JOptionPane.INFORMATION_MESSAGE);
			else
				JOptionPane.showMessageDialog(frame, "ERROR: CD length is non-positive!", cd.getTitle() + " ERROR",
						JOptionPane.ERROR_MESSAGE);
		}
	}
